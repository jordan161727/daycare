...
<aside :class="[collapsed ? 'lg:w-22' : 'lg:w-68', mobileOpen ? 'translate-x-0' : '-translate-x-full lg:translate-x-0']" class="fixed inset-y-0 left-0 z-50 flex w-72 flex-col overflow-hidden bg-gradient-to-b from-blue-700 via-indigo-700 to-violet-800 text-white shadow-2xl transition-all duration-300 lg:sticky lg:top-0 lg:h-screen">
    <div class="flex h-20 items-center justify-between px-5">
        <a href="{{ route('dashboard') }}" class="flex min-w-0 items-center gap-3">
            ...
            <span x-show="!collapsed" class="min-w-0"><span class="block text-lg font-bold">Daycare</span><span class="block text-xs text-blue-200">Management</span></span>
        </a>
        <button @click="collapsed = !collapsed" class="hidden rounded-lg p-2 text-blue-100 hover:bg-white/15 lg:block" aria-label="Collapse sidebar"><svg class="h-5 w-5" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M4 6h16M4 12h16M4 18h16"/></svg></button>
    ...
    <div class="border-t border-white/15 p-4">
        <div class="flex items-center gap-3 rounded-xl p-2"><span class="grid h-9 w-9 shrink-0 place-items-center rounded-full bg-white/20 font-semibold">{{ strtoupper(substr(auth()->user()->name ?? 'Teacher', 0, 1)) }}</span><span x-show="!collapsed" class="min-w-0"><span class="block truncate text-sm font-semibold">{{ auth()->user()->name ?? 'Teacher Name' }}</span><span class="block text-xs text-blue-200">Teacher</span></span></div>
        <button class="nav-link mt-2 w-full"><svg class="h-5 w-5 shrink-0" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M17 16l4-4m0 0l-4-4m4 4H7m4 8H5a2 2 0 01-2-2V6a2 2 0 012-2h6"/></svg><span x-show="!collapsed">Logout</span></button>
    </div>
</aside>
