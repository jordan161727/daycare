...
@section('content')
    ...
    <section class="mt-6"><h2 class="text-lg font-bold">Quick actions</h2><div class="mt-4 grid gap-4 sm:grid-cols-2 xl:grid-cols-4"><x-quick-action href="{{ route('children.import.form') }}" title="Import Children" text="Upload a class roster" icon="upload" /><x-quick-action href="{{ route('attendance.index') }}" title="Attendance" text="Mark today's attendance" icon="check" /><x-quick-action href="{{ route('reports.index') }}" title="Reports" text="View attendance trends" icon="chart" /><x-quick-action href="{{ route('attendance.index') }}" title="History" text="Review sign-in records" icon="clock" /></div></section>
@endsection
