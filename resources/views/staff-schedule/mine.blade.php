...
@php
    ...
    // @php() whose expression carries nested parentheses, and round() inside
...
@else
    ...
    <div class="mt-5 grid gap-4 sm:grid-cols-2 xl:grid-cols-3">
        @foreach($dates as $dayCode => $date)
            ...
            <section class="glass-card rounded-2xl p-5 {{ $isToday ? 'ring-2 ring-indigo-500' : '' }}">
                ...
                @forelse($shifts as $shift)
                    @php($tone = $roomColour[$shift->classroom] ?? '#64748b')
